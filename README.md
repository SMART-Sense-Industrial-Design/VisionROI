...
- [🧾 ชีทสรุปคำสั่งหลัก](#ชีทสรุปคำสั่งหลัก)
...
- รวมผลลัพธ์ ROI ที่ประมวลผลในเฟรมเดียวกันให้เป็น payload เดียว ส่งออกทั้งผ่าน WebSocket (`/ws_roi_result/<cam_id>`) และบันทึกลง `logs/log_<source>/aggregated_roi.log` ด้วย prefix `AGGREGATED_ROI`
...
### ทำไม `ws_snapshot` ถึงเลือกใช้เฟรมจากแคช

เอ็นด์พอยต์ `GET /ws_snapshot/<cam_id>` จำเป็นต้องตอบสนองรวดเร็วเพื่อให้ผู้ใช้ดูภาพตัวอย่างได้ทันทีโดยไม่กระทบสตรีมหลักที่ worker กำลังดึงไปประมวลผล เราจึงปรับให้ลอจิกของมันพยายามใช้ “เฟรมล่าสุดในแคช” ก่อนจะไปขอเฟรมใหม่จากคิวของ `CameraWorker` ซึ่งมีข้อดีดังนี้:

1. **ไม่แย่งคิวกับงานประมวลผลหลัก** – การอ่านจากแคชเป็นการเข้าถึงแบบไม่ทำลาย (non-destructive) ทำให้เฟรมที่คิวเตรียมไว้สำหรับ inference ยังอยู่ครบ ลดความเสี่ยงที่ worker จะว่างงานหรือ inference delay
2. **ลดเวลารอตอบกลับ** – หากมีเฟรมสดในแคชอยู่แล้ว HTTP response สามารถตอบกลับได้ทันที เหมาะกับการกดรีเฟรชดูภาพนิ่งหรือสร้างพรีวิวระหว่างตั้งค่า ROI
3. **รองรับสถานการณ์ที่ worker ยังไม่พร้อม** – เมื่อ worker เพิ่งเริ่มหรือยังไม่ได้ใส่เฟรมลงคิว เราจะได้สถานะที่ชัดเจน (503) พร้อมข้อความอธิบาย สื่อสารกับผู้ใช้งานได้ดีกว่า 500 Internal Server Error
4. **เปิดทางให้ endpoint อื่น reuse ได้** – การมีเมธอด `CameraWorker.get_latest_frame()` ทำให้ส่วนอื่น ๆ สามารถนำเฟรมล่าสุดไปใช้งานได้โดยไม่ต้องสร้างคิวใหม่ เช่น การทำ thumbnail หรือ health check เฉพาะกิจ

> ℹ️ การบันทึกเฟรมล่าสุดไว้ใน `_latest_frame` ไม่ได้ทำให้ลูป inference หรือ ROI ขาดเฟรมไปไหน เฟรมทุกชุดยังถูกนำเข้า `read_and_queue_frame()` เพื่อเข้าคิว ROI/inference ตามปกติ ส่วนการดึง snapshot จะ copy จาก `_latest_frame` ก่อน เมื่อไม่พร้อมจึง fallback ไปอ่านจากคิว ทำให้ได้ภาพครบทุกเฟรมพร้อมผล OCR ของทุก ROI เหมือนเดิม เพียงแต่ลดการแย่งคิวกับงาน inference ลงเท่านั้น

โดยรวมแล้ววิธีนี้ช่วยให้การแสดง snapshot ไวขึ้น เสถียรกว่า และลดผลกระทบต่อสตรีมหลักที่ต้องใช้ในการ inference หรือเผยแพร่แบบเรียลไทม์

#### ผลกระทบต่อสตรีมหลักและการ inference

- ลูปหลักใน `CameraWorker` เพียงเก็บอ้างอิงเฟรมล่าสุดไว้ในตัวแปร `_latest_frame` โดยไม่ทำสำเนา จึงไม่มีงานเพิ่มต่อเฟรมใด ๆ และยังคงส่งเฟรมเข้า queue สำหรับ inference ได้ต่อเนื่อง
- การเรียก `ws_snapshot` จะทำสำเนาเฟรมก็ต่อเมื่อมีคำขอ HTTP เข้ามาเท่านั้น และการ copy นั้นเกิดนอกลูปหลัก ทำให้ throughput ของสตรีม inference ไม่ถูกหน่วง
- เมื่อไม่มีเฟรมในแคช ระบบจะ fallback ไปอ่านผ่าน `worker.read()` เช่นเดิมและตอบกลับ `503` หากยังไม่พร้อม ช่วยให้ snapshot ไม่ดึงคิวร่วมกับงาน inference นานเกินไปและไม่ทำให้ลูปหลักรอคอย
...
> 💡 **ทิป:** หากต้องการควบคุมสาขาที่จะโคลน ให้ตั้งค่าตัวแปร `GIT_BRANCH` ภายในสคริปต์ก่อนรัน (ค่าเริ่มต้นคือ `main`). สำหรับสภาพแวดล้อมที่ไม่มีสิทธิ์ `sudo`
> สามารถคอมเมนต์บรรทัดติดตั้ง `apt` ออก แล้วติดตั้ง dependencies ด้วยตนเองแทน
...
> หากวางแผนใช้ GPU กับโมดูลที่พึ่งพา PyTorch หรือ TensorRT ควรตรวจสอบว่าระบบมีไดรเวอร์ CUDA ที่รองรับเวอร์ชันของไลบรารีดังกล่าว และติดตั้งแพ็กเกจเสริม
> เช่น `cuda-toolkit`, `cudnn` หรือ `tensorrt` ตามคู่มือของผู้ผลิตฮาร์ดแวร์

### ตัวแปรสภาพแวดล้อมที่สำคัญ
| ตัวแปร | ค่าเริ่มต้น | คำอธิบาย | หมายเหตุ |
| --- | --- | --- | --- |
| `INFERENCE_RESULT_TIMEOUT` | `10.0` | เวลารอผลลัพธ์จากโมดูล inference ต่อ ROI (วินาที) | ใช้ควบคุมการตัดการเชื่อมต่อของ WebSocket หากโมดูลประมวลผลนาน สามารถ override เฉพาะงานผ่านพารามิเตอร์ `result_timeout` ได้ |
| `STREAM_MAX_FRAME_DELAY` | `1.5` | อายุเฟรมสูงสุดก่อนถูกดรอป (วินาที) ขณะสตรีมผ่าน WebSocket | กำหนด `0` เพื่อปิดการตัดเฟรมสะสม เหมาะเมื่อเน้นความครบถ้วนมากกว่าความสดใหม่ |
| `STREAM_SEND_TIMEOUT` | `0.6` | เวลารอสูงสุดขณะส่งข้อมูลผ่าน WebSocket (วินาที) | หากเครือข่ายหน่วงจนเกินค่านี้ ระบบจะบันทึกคำเตือนและปิดการเชื่อมต่อเพื่อป้องกันงานค้าง |

ค่าดีฟอลต์ครอบคลุมการใช้งานทั่วไป แต่สามารถตั้งค่าตามสภาพแวดล้อมจริงได้ เช่น เพิ่ม `INFERENCE_RESULT_TIMEOUT` เมื่อต้องเรียกโมเดลขนาดใหญ่ หรือปรับ `STREAM_MAX_FRAME_DELAY` ให้สูงขึ้นสำหรับการเชื่อมต่อที่มีแบนด์วิดท์จำกัด

#### การปรับแต่ง RapidOCR
- ตั้งค่า `RAPIDOCR_ORT_PROVIDERS` (ตัวอย่าง `"TensorrtExecutionProvider,CUDAExecutionProvider,CPUExecutionProvider"`) เพื่อเรียงลำดับ Execution Provider ของ onnxruntime ตามโครงสร้าง GPU ที่มีอยู่
- หากใช้งาน TensorRT ให้กำหนด `ORT_TENSORRT_ENGINE_CACHE_PATH` ไปยังพาธที่ต้องการเก็บ cache (เช่น SSD ภายในเครื่อง) เพื่อเร่งรอบเริ่มต้นของโมดูล RapidOCR
...
### โหมดพัฒนาด้วย Hot Reload
ระหว่างพัฒนา UI หรือโมดูล inference การรีโหลดเซิร์ฟเวอร์อัตโนมัติช่วยให้รอบการปรับแก้รวดเร็วขึ้น สามารถเลือกวิธีต่อไปนี้ตามความถนัด:

- ใช้คำสั่งของ Quart โดยตรง:

  ```bash
  quart --app app:app run --reload --port 5000
  ```

  คำสั่งนี้จะติดตามการแก้ไขไฟล์ Python ภายในโปรเจ็กต์แล้วรีสตาร์ทเซิร์ฟเวอร์อัตโนมัติ

- ใช้ `watchfiles` เพื่อควบคุมการรีโหลดละเอียดขึ้น (ติดตั้งด้วย `pip install watchfiles`):

  ```bash
  watchfiles --filter python 'python app.py'
  ```

  วิธีนี้เหมาะเมื่อจำเป็นต้องรันคำสั่งเริ่มเซิร์ฟเวอร์แบบกำหนดเองหรือเพิ่มอาร์กิวเมนต์พิเศษให้ `app.py`

เมื่อเปิดใช้งานโหมด hot reload ควรตรวจสอบ log เป็นระยะ หากโมดูลบางตัวโหลดนานอาจทำให้รีโหลดช้าหรือเกิดการชนระหว่าง worker ได้
...
<a id="ชีทสรุปคำสั่งหลัก"></a>
## 🧾 ชีทสรุปคำสั่งหลัก

สำหรับผู้ใช้ที่ต้องการอ้างอิงคำสั่งสำคัญอย่างรวดเร็ว ตารางด้านล่างรวบรวมขั้นตอนพื้นฐานที่ใช้บ่อยระหว่างการเตรียมสภาพแวดล้อม การพัฒนา และการตรวจสุขภาพของระบบ:

| หมวด | คำสั่ง | จุดประสงค์ |
| --- | --- | --- |
| เตรียม Virtual Environment | `python -m venv .venv`<br>`source .venv/bin/activate` | สร้างและเปิดใช้งาน virtual environment (บน Windows ใช้ `.venv\\Scripts\\activate`) |
| ติดตั้งแพ็กเกจ | `pip install "."`<br>`pip install ".[server,onnx]"` | ติดตั้งโปรเจ็กต์หลักและ extras ที่ต้องการ เช่น server/onnx |
| รันเซิร์ฟเวอร์ | `python app.py --use-uvicorn --port 12000` | เริ่มแอปด้วย Uvicorn และกำหนดพอร์ตเอง |
| ตรวจสุขภาพบริการ | `curl http://localhost:5000/_healthz` | ตรวจสอบว่าแอปตอบสนองและ worker ทำงานปกติ |
| ปิดบริการอย่างปลอดภัย | `curl -X POST http://localhost:5000/_quit` | หยุดเซิร์ฟเวอร์เมื่อทดสอบเสร็จ |
| รันการทดสอบ | `pytest` | ตรวจสอบความถูกต้องของฟีเจอร์หลักผ่านยูนิตเทสต์ |

> สามารถคัดลอกตารางไปติดไว้ในสคริปต์หรือเอกสารภายในทีม เพื่อช่วยให้สมาชิกใหม่เริ่มใช้งาน VisionROI ได้รวดเร็วยิ่งขึ้น
...
3. เปิดหน้า `/roi` แล้วเลือกตำแหน่ง ROI ที่ต้องการ จากนั้นกดบันทึก (ROI แต่ละจุดสามารถระบุโมดูลของตัวเองได้) – เรียก `POST /save_roi`
   - ระบบอนุญาตให้บันทึก ROI ที่ยังมีจุดไม่ครบ 4 จุดได้ แต่ ROI ลักษณะดังกล่าวจะไม่ถูกนำไปรวมในสถิติแดชบอร์ดหรือใช้ inference OCR จนกว่าจะกำหนดจุดให้ครบรูปสี่เหลี่ยม
...
- Payload ที่ส่งเป็น JSON ประกอบด้วย `cam_id`, `source`, `group`, `roi_id`, `roi_name`, `module`, `text`, `frame_time`, `result_time` และอาจมี `duration` หากโมดูลส่งค่ามา พร้อมบันทึกลง log `logs/log_<source>/aggregated_roi.log` (ภายใต้ prefix `AGGREGATED_ROI`) และเก็บสรุปไว้ใน `recent_notifications`
...
- **GET `/read_log`** – อ่านบรรทัดล่าสุดจากไฟล์ `logs/log_<source>/aggregated_roi.log` ของ source ที่ระบุผ่านพารามิเตอร์ `source` (สามารถกำหนดจำนวนบรรทัดด้วย `lines`, ค่าเริ่มต้น 40). เมื่อระบบบันทึกผลลัพธ์แบบรวมจะขึ้นต้นด้วย `AGGREGATED_ROI ` แล้วตามด้วย JSON ที่มีโครงสร้างเดียวกับข้อความจาก WebSocket ทำให้สามารถ parse เพื่อวิเคราะห์ย้อนหลังได้ง่าย
...
- **WebSocket `/ws_roi_result/<cam_id>`** – ส่งข้อมูลผลลัพธ์ของงาน inference ตามสถานะปัจจุบัน โดยมีสองรูปแบบข้อความหลัก
  ...
  แต่ละข้อความอาจมี `image` เป็น Base64 JPEG (หากตั้งให้บันทึก ROI), ระยะเวลา `duration` ที่โมดูลใช้ประมวลผล, ค่ารวม `frame_duration` ของเฟรมทั้งภาพ และชื่อคอนฟิก MQTT หากมีการเผยแพร่ผลลัพธ์ไปยัง broker เดียวกัน ขณะที่ไฟล์ `logs/log_<source>/aggregated_roi.log` จะเก็บข้อมูลแบบย่อภายใต้ prefix `AGGREGATED_ROI` โดยคงไว้เฉพาะฟิลด์สำคัญ เช่น `id`, `name`, `text`, `module`, `duration` และ `frame_duration` (ถ้ามี) เพื่อให้ log มีขนาดเหมาะสม
...
- `logs/log_<name>/aggregated_roi.log` ใช้สำหรับบันทึกผลลัพธ์รวมของแต่ละ source โดยทุกบรรทัดขึ้นต้นด้วย `AGGREGATED_ROI ` ตามด้วย JSON แบบย่อ (ฟิลด์หลัก เช่น `id`, `name`, `text`, `module`, `duration`) เพื่อให้อ่านย้อนหลังได้ง่ายและไม่ปะปนกับ log ของโมดูล inference
- `logs/log_<source>/custom.log` จะเก็บ log ของโมดูล inference (`rapid_ocr`, `easy_ocr`, `yolo`, ฯลฯ) ที่รันกับ source นั้น ๆ (เหมาะสำหรับ debug pipeline)
...
### เช็กลิสต์ก่อนส่ง Pull Request
ก่อนเปิด PR ใหม่ แนะนำให้ตรวจสอบหัวข้อด้านล่างเพื่อช่วยให้การรีวิวราบรื่นและลดการย้อนแก้:

1. ✅ เทสต์ทั้งหมดผ่าน (`pytest`) และไม่มีคำสั่งค้างคาใน `watchfiles` หรือ `quart --reload`
2. ✅ รัน `python -m compileall` เพื่อตรวจสอบ SyntaxError ในโมดูลที่แก้ไข
3. ✅ อัปเดตเอกสารหรือคอมเมนต์ในโค้ด หากมีการเปลี่ยนแปลงพฤติกรรมของ API หรือโครงสร้างข้อมูล
4. ✅ ตรวจสอบ log ใน `logs/log_<source>/custom.log` หรือ log เฉพาะโมดูล เพื่อยืนยันว่าการปรับแต่งไม่สร้าง warning ใหม่
5. ✅ เติมคำอธิบายชัดเจนใน PR และเชื่อมโยง issue ที่เกี่ยวข้อง (ถ้ามี)
...
ทุกโมดูลสามารถเขียนข้อความลงไฟล์ `logs/log_<source>/custom.log` ระหว่างรันงาน เมื่อพบปัญหาให้เปิดไฟล์ดังกล่าวหรือตั้งค่าให้ระบบส่ง log ไปยัง MQTT
