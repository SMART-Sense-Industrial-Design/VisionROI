...
- ตรวจจับหน้ากระดาษและ ROI แบบเพจผ่านหน้า Document (`/inference_page`)
...
4. หากต้องการตรวจจับหน้ากระดาษ ให้เข้า Document ที่ `/inference_page` เพื่อดูผลลัพธ์การตรวจจับเพจ
5. เข้า `/inference` แล้วเลือก source กล้องเพื่อเริ่มประมวลผล ผลลัพธ์แต่ละ ROI จะถูกส่งกลับผ่าน `/ws_roi_result/<cam_id>` โดยที่ `cam_id` เป็นคีย์สตริง (เช่น `inf_cam1`)
...
## หน้าต่างต่าง ๆ
### Home (`/home`)
หน้าแรกแสดงแดชบอร์ดสรุปฟีเจอร์หลักของระบบ ได้แก่ **Create Source**, **ROI** และ **Inference** เพื่อให้ผู้ใช้เข้าใจขั้นตอนการทำงานโดยรวมตั้งแต่จัดการแหล่งวิดีโอไปจนถึงรันโมเดล AI นอกจากนี้เมื่อเข้าใช้งานครั้งแรกจะมีข้อความต้อนรับแสดงขึ้นมา
