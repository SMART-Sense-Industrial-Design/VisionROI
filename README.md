...
> 💡 **ผู้ใช้ macOS (กล้องผ่าน `avfoundation`):** VisionROI จะสั่งให้ `ffmpeg` ใช้ pixel format `nv12` ให้อัตโนมัติ ซึ่งรองรับกับกล้องส่วนใหญ่ของระบบปฏิบัติการนี้
> และหากอุปกรณ์ไม่รองรับ VisionROI จะอ่าน log ของ `ffmpeg` แล้วสลับไปยัง pixel format อื่นที่อุปกรณ์รายงานว่ารองรับให้อัตโนมัติ
   > หากกล้องของคุณรองรับฟอร์แมตอื่น (เช่น `uyvy422` หรือ `yuyv422`) ให้ตั้งค่าตัวแปรสภาพแวดล้อม `FFMPEG_AVFOUNDATION_PIXEL_FORMAT`
   > ก่อนรันแอป เช่น `export FFMPEG_AVFOUNDATION_PIXEL_FORMAT=uyvy422` เพื่อให้สตรีมเปิดได้สำเร็จ
