...
def tlwh_to_xyxy(tlwh):
    """
    Convert `tlwh` (top-left x, top-left y, width, height) to `xyxy` (x1, y1, x2, y2).
    Ensure x2 and y2 are calculated correctly.
    """
    x, y, w, h = tlwh
    x1 = x
    y1 = y
    x2 = x + w  # จุดขวาสุด: ต้องบวกความกว้าง
    y2 = y + h  # จุดล่างสุด: ต้องบวกความสูง
    
    # Debug: พิมพ์ค่าเพื่อตรวจสอบความถูกต้อง
    # print(f"tlwh: {tlwh}, xyxy: [{x1}, {y1}, {x2}, {y2}]")
...
class BoTSORT(object):
    ...
    def update(self, output_results, img):
        ...
        if len(dets) > 0:
            '''Detections'''
            if self.args.with_reid:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, cls=c, feat=f) for
        ...
        for t in output_stracks:
            output = []
            xyxy = tlwh_to_xyxy(t.tlwh)  # ใช้ฟังก์ชันที่ไม่ปรับ scale
            output.extend(xyxy)
